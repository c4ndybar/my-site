...
async function truncateRefAndInsertItems(ref: FirebaseFirestore.CollectionReference, items: any[]) {
    let batch = db.batch();
...
async function getInstagramAccessToken(): Promise<InstagramToken> {
    const tokenData: any = (await db.collection('keys').doc('instagram').get()).data();
